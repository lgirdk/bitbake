...
def emit_func(func, o=sys.__stdout__, d = init()):
    ...
    o.write('\n')
    keys = (key for key in d.keys() if not key.startswith("__") and not d.getVarFlag(key, "func", False) and not d.getVarFlag(key, 'unexport', False))
    for key in sorted(keys):
        emit_var(key, o, d, False)

    o.write('\n')
    keys = (key for key in d.keys() if not key.startswith("__") and not d.getVarFlag(key, "func", False) and d.getVarFlag(key, 'unexport', False))
    for key in sorted(keys):
        emit_var(key, o, d, False)
